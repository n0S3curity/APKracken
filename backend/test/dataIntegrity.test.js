...
test('scan deletion blocks a running scan unless forced, and ignores orphaned metadata', async () => {
  const mutations = [];
  const base = {
    $queryRaw: async () => [],
    ...
    stepMetadata: { deleteMany: async () => mutations.push('stepMetadata') },
    postProcessMetadata: { deleteMany: async () => mutations.push('postMetadata') },
  };

  // A running (non-terminal) scan is protected from a normal delete.
  const nonTerminal = {
    ...base,
    scan: {
      findUnique: async () => ({ id: 9n, status: 'running' }),
      delete: async () => mutations.push('scan'),
      update: async () => mutations.push('scan.update'),
    },
  };
  assert.deepEqual(await deleteScanIfSafe(nonTerminal, 9n), { kind: 'not-terminal', status: 'running' });
  assert.deepEqual(mutations, []);

  // A terminal scan with an orphaned `running` metadata row (crashed worker) must
  // still delete — the old guard made such scans permanently undeletable.
  const orphaned = {
    ...base,
    scan: { findUnique: async () => ({ id: 9n, status: 'stopped' }), delete: async () => mutations.push('scan') },
  };
  assert.deepEqual(await deleteScanIfSafe(orphaned, 9n), { kind: 'deleted' });
  assert.ok(mutations.includes('scan'));

  // Forcing a running scan stops it (scan.update -> stopped) before deleting.
  mutations.length = 0;
  const forced = {
    ...base,
    scan: {
  ...
  assert.deepEqual(await deleteScanIfSafe(forced, 9n, { force: true }), { kind: 'deleted' });
  assert.ok(mutations.includes('scan.update'));
  assert.ok(mutations.includes('scan'));
