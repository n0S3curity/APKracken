...
test('configuredModelProviders returns canonical providers configured by presence flags', () => {
  ...
  // 'local' is the on-host llama-server — always available, no credential.
  assert.deepEqual(providers, ['codex', 'claude', 'openrouter', 'local']);
});

test('configuredModelProviders does not mistake a stale Codex login marker for credentials', () => {
  assert.deepEqual(configuredModelProviders({ env: { OPEN_KRITT_CODEX_LOGIN_CONFIGURED: '1' } }), ['local']);
  assert.deepEqual(configuredModelProviders({ env: { CODEX_LOGIN_CONFIGURED: 'true' } }), ['local']);
});
...
test('configuredModelProviders does not treat disabled presence flags as credentials', () => {
  ...
  // Only the always-on local provider remains when no API credentials are set.
  assert.deepEqual(providers, ['local']);
});
...
test('model provider API exposes configured IDs and rejects unavailable scan providers', async (t) => {
  ...
  // The local provider is always offered alongside any credential-backed ones.
  assert.deepEqual(availability.body, { providers: ['codex', 'local'] });
