...
const PROVIDER_CREDENTIALS = {
  ...
  // The local llama-server needs no API key; it is configured on the Accounts
  // page (base_url/model_name) and is always offered as a provider.
  local: [],
};
...
export function configuredModelProviders({
  ...
  return MODEL_PROVIDERS.filter((provider) => {
    // The local provider is the on-host llama-server — always available, no key.
