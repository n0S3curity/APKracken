...
// `exploitable` as the model self-reported it is an assertion, not a result: the headline
// count read it directly, so a scan showed "15 exploitable" while the adjudicator had already
// rejected two of those findings and marked three as only conditionally reachable. The
// adjudicator's verdict wins where it exists, and a citation the snapshot contradicts never
// counts. `needs_conditions` is deliberately excluded — it is real but chain-dependent, and
// the finding carries its own badge for that.
export function findingIsExploitable(jsonAnswer) {
  if (!jsonAnswer || typeof jsonAnswer !== 'object') return false;
  const verdict = typeof jsonAnswer.fp_verdict === 'string' ? jsonAnswer.fp_verdict.trim().toLowerCase() : '';
  if (verdict === 'exploitable') return true;
  if (verdict === 'false_positive' || verdict === 'needs_conditions') return false;
  if (jsonAnswer.evidence_verdict === 'file_missing') return false;
  const claimed = jsonAnswer.exploitable;
...
export async function findingCountsByScan(scanIds) {
  ...
  for (const v of vulns) {
    ...
    if (findingIsExploitable(v.jsonAnswer)) entry.exploitable += 1;
  }
  return map;
}
