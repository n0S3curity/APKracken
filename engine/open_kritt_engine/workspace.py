...
def _job_identity(metadata_id: int) -> tuple[int, int]:
    # Windows has no POSIX uid/gid; there is no per-job host identity to assign.
...
# An entrypoint sweep costs a few seconds and is identical for every job of a scan, so it is
# memoized on the primary repo+commit rather than recomputed for every job workspace.
_SURFACE_CACHE: dict[str, str] = {}


def _entrypoint_briefing(repo_dir: str, manifest: dict[str, Any]) -> str:
    """Deterministic attack-surface intelligence for SOURCE repos.

    APK workspaces already ship WORKSPACE.json with the exported-component list; source repos
    had no equivalent, so the model had to discover the whole surface by grepping and ran out
    of steps long before it finished. This gives the source flow the same head start."""

    if os.path.isdir(os.path.join(repo_dir, "jadx")):
        return ""  # APK workspace: its manifest intelligence is richer than a pattern sweep
    primary = manifest.get("primary") if isinstance(manifest, dict) else {}
    primary = primary if isinstance(primary, dict) else {}
    key = str(primary.get("repo")) + "@" + str(primary.get("commit") or primary.get("requested_commit"))
    if key in _SURFACE_CACHE:
        return _SURFACE_CACHE[key]
    try:
        from .source_surface import discover_entrypoints, surface_summary

        text = surface_summary(discover_entrypoints(repo_dir))
    except Exception:  # noqa: BLE001 - context enrichment must never break a scan
        LOGGER.exception("entrypoint sweep failed for %s", repo_dir)
        text = ""
    _SURFACE_CACHE[key] = text
    return text
...
def workspace_layout(repo_dir: str, manifest: dict[str, Any]) -> str:
    ...
    briefing = _entrypoint_briefing(repo_dir, manifest)
    if briefing:
        lines.append("")
...
def resolve_scan_snapshot_dir(cache_dir: str, scan: dict[str, Any]) -> str | None:
    """The on-disk snapshot a completed scan was analysed from.

    Per-job workspaces are ephemeral, but the checkout cache survives the run, which is what
    lets the post-scan accuracy pass re-read the exact code the model saw."""

    try:
        bases = _scan_cache_bases(Path(cache_dir), scan)
    except Exception:  # noqa: BLE001
        return None
    for base in bases:
        try:
            ready = _read_ready_cache_checkout(base)
        except Exception:  # noqa: BLE001
            continue
        if ready is not None and Path(ready[0]).is_dir():
            return str(ready[0])
    return None
