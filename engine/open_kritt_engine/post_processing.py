    ...
    def process_once(self, scan: dict[str, Any], harness) -> bool:
        ...
        with self.db.connect() as conn:
            ...
            if not current or current["status"] in {"paused", "stopped", "failed", "completed"}:
                return False
            # APK scans skip the model-driven dedupe/ranker/post-script chain. Those are
            # tuned for cloud models and the local 26B model returns invalid_output on
            # them, which would loop the scan in post_processing forever. The specialist
            # investigation phase (run after completion) does its own dedup + re-ranking,
            # so we go straight to `completed` here and let it take over.
            #
            # Mark every finding as already deduped + ranked so the UI shows them as
            # canonical. The scan-admission query also skips repo_kind='apk' outright
            # (see claim_scans), which is what actually prevents the completed<->post_
            # processing flicker; this keeps the finding rows internally consistent.
            if (current.get("repo_kind") or "remote") == "apk":
                conn.execute(
                    """
                    UPDATE workflows.vulnerabilities
                    SET dedupe_is_canonical = true,
                        bounty_rank = COALESCE(bounty_rank, rank, 0)
                    WHERE scan_id = %s
                      AND (dedupe_is_canonical IS NULL OR bounty_rank IS NULL)
                    """,
                    (scan_id,),
                )
                # Rank by severity so the DAG workflow's findings (which get no investigation
                # phase) are ordered; the investigation phase re-ranks the triage workflow.
                rows = conn.execute(
                ...
                def _score(r):
                    ...
                    try:
                        return float(ja.get("severity_score") or 0)
                    except (TypeError, ValueError):
                        return 0.0
                for rk, r in enumerate(sorted(rows, key=_score, reverse=True), 1):
                    vid = r[0] if not isinstance(r, dict) else r["id"]
                    conn.execute("UPDATE workflows.vulnerabilities SET rank = %s WHERE id = %s", (rk, vid))
