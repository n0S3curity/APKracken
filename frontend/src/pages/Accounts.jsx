...
const SOURCE_LABELS = {
  managed_api_key: 'Managed in APKraken',
...
export default function Accounts() {
  ...
  const remove = async (provider) => {
    if (!(await confirm({ title: 'Remove API key', message: 'Remove the OpenRouter API key from APKraken?', confirmLabel: 'Remove', danger: true }))) return;
    const previous = data;
  ...
  const removeLoginAccount = async (provider, account) => {
    ...
    if (!(await confirm({ title: `Remove ${label}?`, message: impact, confirmLabel: 'Remove', danger: true }))) return;
  ...
  const useManualReset = async (account) => {
    ...
    const label = account.email || account.label;
    if (!(await confirm({ title: 'Use a manual reset?', message: `Use 1 of ${available} manual resets for ${label}?\n\nThis cannot be undone.`, confirmLabel: 'Use reset', danger: true }))) return;
    setError(null);
          ...
          <div className="account-login-intro">
            {provider.id === 'codex' ? (
              <>
                ...
                ChatGPT account you want APKraken to use.
              </>
            ) : (
              <>
...
// ---------------------------------------------------------------------------
// Local model (llama.cpp) control — config + live status + restart.
// ---------------------------------------------------------------------------
const LOCAL_STATUS = {
  running: { color: 'var(--ok)', label: 'Running' },
  starting: { color: 'var(--pend)', label: 'Starting…' },
  stopped: { color: 'var(--fail)', label: 'Stopped' },
  error: { color: 'var(--fail)', label: 'Error' },
...
function LocalModelCard() {
  ...
  const refresh = useCallback(async ({ keepForm } = {}) => {
    try {
      const next = await api.localModel();
      setCfg(next);
      if (!keepForm) {
        setForm((prev) => prev || {
  ...
  useEffect(() => {
    refresh();
    const t = setInterval(() => refresh({ keepForm: true }), 3000);
    return () => clearInterval(t);
  }, [refresh]);

  if (!cfg || !form) return null;
  const status = LOCAL_STATUS[cfg.status] || LOCAL_STATUS.unknown;
  const set = (k, v) => {
    setForm((f) => ({ ...f, [k]: v }));
    setSaved(false);
  };
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const next = await api.updateLocalModel(form);
      setCfg(next);
      setSaved(true);
    } catch (e) {
      setError(e);
  ...
  const restart = async () => {
    if (!(await confirm({
      title: 'Restart local model',
      message: 'Restart the local model (llama.cpp)? In-flight scans on the local model will fail and need resuming, and the model takes ~30–60s to reload.',
    ...
    setError(null);
    try {
      const next = await api.restartLocalModel();
      setCfg(next);
    } catch (e) {
      setError(e);
    } finally {
  ...
  const stop = async () => {
    if (!(await confirm({
      ...
      message: 'Stop the local model (llama.cpp)? It stays stopped until you press Start, and any scans on the local model will fail meanwhile.',
      confirmLabel: 'Stop',
      danger: true,
    }))) return;
    setBusy(true);
    setError(null);
    try {
      setCfg(await api.stopLocalModel());
    } catch (e) {
      setError(e);
    } finally {
      setBusy(false);
  ...
  const start = async () => {
    ...
    setError(null);
    try {
      setCfg(await api.startLocalModel());
    } catch (e) {
      setError(e);
    } finally {
      setBusy(false);
  ...
  const isStopped = cfg.status === 'stopped' || cfg.desiredRunning === false;

  const field = { display: 'flex', flexDirection: 'column', gap: 5 };
  const label = { fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.04em' };
  const input = {
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 14, background: 'var(--surface)', padding: 20, margin: '18px 0 8px', boxShadow: 'var(--shadow)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 16, fontWeight: 600 }}>Local model</span>
          <span title={cfg.statusDetail || status.label}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 600, color: status.color }}>
            <span style={{ width: 9, height: 9, borderRadius: '50%', background: status.color, boxShadow: cfg.status === 'running' ? `0 0 0 3px color-mix(in srgb, ${status.color} 22%, transparent)` : 'none' }} />
            {status.label}
          </span>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Button variant="subtle" style={{ height: 32 }} onClick={save} disabled={busy}>
            {busy ? '…' : saved ? 'Saved ✓' : 'Save'}
          </Button>
          {isStopped ? (
            <Button variant="primary" style={{ height: 32 }} onClick={start} disabled={busy}>
              Start model
            </Button>
          ) : (
            <>
              <Button variant="subtle" style={{ height: 32 }} onClick={stop} disabled={busy}>
                Stop
              </Button>
              <Button variant="danger" style={{ height: 32 }} onClick={restart} disabled={busy}>
                Restart model
              </Button>
            </>
          )}
      ...
      <div style={{ fontSize: 12.5, color: 'var(--text-2)', marginBottom: 16, lineHeight: 1.5 }}>
        llama.cpp is managed by the engine. Edits apply to the next model call; changing paths or the model takes effect on Restart.
      </div>
      {error && <ErrorState error={error} />}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
        <label style={field}>
          <span style={label}>Address URL &amp; port</span>
          <input style={input} value={form.baseUrl} onChange={(e) => set('baseUrl', e.target.value)} placeholder="http://127.0.0.1:8005/v1" />
        </label>
        <label style={field}>
          <span style={label}>Model name</span>
          <input style={input} value={form.modelName} onChange={(e) => set('modelName', e.target.value)} placeholder="local" />
        </label>
        <label style={field}>
          <span style={label}>Reasoning effort</span>
          <select style={input} value={form.reasoningEffort} onChange={(e) => set('reasoningEffort', e.target.value)}>
      ...
      <details>
        <summary style={{ cursor: 'pointer', fontSize: 12, color: 'var(--text-3)' }}>Advanced (llama.cpp paths + autostart)</summary>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 12, marginTop: 12 }}>
          <label style={field}>
            <span style={label}>llama.cpp directory</span>
            <input style={input} value={form.llamaDir || ''} onChange={(e) => set('llamaDir', e.target.value)} />
          </label>
          <label style={field}>
            <span style={label}>Model file (.gguf)</span>
            <input style={input} value={form.modelFile || ''} onChange={(e) => set('modelFile', e.target.value)} />
          </label>
          <label style={field}>
            <span style={label}>mmproj file</span>
            <input style={input} value={form.mmprojFile || ''} onChange={(e) => set('mmprojFile', e.target.value)} />
          </label>
        </div>
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 12, fontSize: 12.5, color: 'var(--text-2)' }}>
          <input type="checkbox" checked={!!form.autostart} onChange={(e) => set('autostart', e.target.checked)} style={{ accentColor: 'var(--accent)' }} />
          Auto-start the model if it isn&apos;t running
        </label>
      </details>
    </div>
  );
